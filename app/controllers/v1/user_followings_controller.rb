class V1::UserFollowingsController < ApplicationController
  ...
  def show
	...
	users = []
	user_followings.each do |u_f|
		users << User.find(u_f.user_id)
	end
	render json: users, status: :ok
  end

  def create
	if UserFollowing.where(user_id: params[:user_id]).where(following_user_id: params[:following_user_id]).presents?
		render json: user_following, message: "Already Following", status: :unprocessable_entity
	end
  ...
  def destroy
  	user_following = UserFollowing.where(user_id: params[:user_id]).where(following_user_id: params[:following_user_id])
  ...
  private
	def following_param
		params.require(:user_following).permit(:user_id,
									 		   :following_user_id)
	end
